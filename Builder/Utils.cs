...
public static class ExtensionsForNuke
{
   ...
   public static async Task UploadXmlNunit3TestResultsAsync(this AbsolutePath folder)
   {
      var jobId = AppVeyor.Instance?.JobId;
      var testFiles = folder.GlobFiles("*.xml");
      foreach( var testFile in testFiles )
      {
         Log.Information($"Uploading test result file {testFile}");
         var response = await $"https://ci.appveyor.com/api/testresults/nunit3/{jobId}"
                          .PostMultipartAsync(mp =>
                          {
                             mp.AddFile("file", testFile);
                          });

         Assert.True(response.StatusCode == (int)HttpStatusCode.NoContent, $"Failed to upload test results to AppVeyor; HTTP Status Code: {response.StatusCode}");
         Log.Information($"Test file '{testFile}' upload ok");
      }
   }
}
